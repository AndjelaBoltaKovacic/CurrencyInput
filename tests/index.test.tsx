...
describe('CurrencyInput', () => {
    it('renders without crashing', () => {
        render(<CurrencyInput onChange={() => { }} type="text" />);
    });

    it('displays the default value correctly', () => {
        const { getByDisplayValue } = render(
            <CurrencyInput onChange={() => { }} defaultValue="1234.56" type="text" />
        );
    ...
    it('calls the onChange function with the correct value when input changes', () => {
        ...
        const component = render(
            <CurrencyInput onChange={mockOnChange} type="text" />
        );
        const getById = queryByAttribute.bind(null, 'id');
        const inputElement = getById(component.container, 'currency-input'); // Use getById to select by id attribute
        expect(inputElement).toBeInTheDocument();
